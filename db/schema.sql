...
create policy "authors_admin_all" on public.authors for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

drop policy if exists "articles_admin_all" on public.articles;
create policy "articles_admin_all" on public.articles for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

drop policy if exists "tags_admin_all" on public.tags;
create policy "tags_admin_all" on public.tags for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

drop policy if exists "article_tags_admin_all" on public.article_tags;
create policy "article_tags_admin_all" on public.article_tags for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');

-- View increment RPC (by slug, debounced client-side)
...
-- ============================================================
-- SEED — categories, author
-- ============================================================
insert into public.categories (name, slug, description, sort_order, is_active) values
  ('الهوية البصرية','visual-identity','شعارات وأنظمة تصميم متكاملة بأسلوب أحادي اللون.',1,true),
  ('تطوير الواجهات','frontend','واجهات سريعة ومدروسة مبنية بأفضل الممارسات.',2,true),
  ('تجربة المستخدم','ux','تصميم رحلات استخدام واضحة ومدروسة.',3,true),
  ('الفلسفة','philosophy','أفكار حول الوضوح والذوق والبساطة.',4,true)
on conflict (slug) do nothing;

insert into public.authors (name, slug, bio) values
  ('فريق مُوكْرُو','mocro-team','نكتب عن التصميم الرقمي بهوية واضحة وبساطة مقصودة.')
on conflict (slug) do nothing;